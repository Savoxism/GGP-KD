...
This observation suggests a different principle: \emph{a frozen student
retriever should determine which relations are eligible for supervision, the
teacher should determine their targets, and mini-batches should only realize
them efficiently.}

Based on this principle, we introduce \textsc{GGPKD}, a framework for
student-selected, teacher-valued relational distillation. GGPKD encodes the
corpus once with both the teacher and frozen base student. Student $k$NN selects
each row's columns; teacher cosines on those columns define the transition
target, while the teacher's own retrieval span defines its bandwidth. The
supervision support is therefore fixed and identical in every epoch.
Mini-batches enter only as a computational device:
...
across the pool, and supervising the graph-selected rows of non-anchor pool
...
Two explicit non-negative coefficients weight the relational groups directly,
without normalization. Together with graph width and the auxiliary-row
coefficient, this introduces four independent method-specific degrees of freedom.
...
\begin{itemize}
    ...
    is fixed by frozen-student retrieval and valued by the teacher, from
    relation realization, which is
    ...
    cached student-$k$NN, teacher-valued graph, per-row bandwidths derived from
    teacher retrieval spans, deterministic row-shaped supports, and amortized
    auxiliary-row supervision that reuses already-encoded pool members. The
    relational groups use explicit independent coefficients, giving four
    method-specific degrees of freedom including graph width and row weighting.
    ...
    results on semantic textual similarity, and show that its explicit
    structural and loss settings are flat enough that the defaults are not
